...
public class ReflectionScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Get reflection data
        string reflectionQ = getData();

        // Set reflection question on the screen
        GameObject.FindGameObjectWithTag("ScreenText").GetComponent<TextMeshProUGUI>().text = reflectionQ;
    ...
    private void OnMessage(int device_id, JToken data)
    {
        ...
        if (data != null && AirConsole.instance.IsAirConsoleUnityPluginReady())
        {
            ...
            if (data["element"] != null && data["element"].ToString() == "view-9-section-0-element-0")
            {
    ...
    {
        // reflection data
        List<string> dataList = GameObject.FindGameObjectWithTag("GameLogic").GetComponent<GamesData>().getReflectionData();
        List<string> shuffledData = dataList.OrderBy(x => UnityEngine.Random.value).ToList();
        string data = shuffledData[0];
        return data;
    }
}
